...
class RqTableFetcher:
    """Fetches data from GitHub API, store and return the data in a RqTable."""
    ...
    def Fetch(self, label):
        ret = tb.RqTable()
        org_name, sub_name, add_info = self._Parse(label) # to break google.issues.something to org_name = 'google' sub_name = 'issues' add_info = 'something'
        org = self._github.get_organization(org_name)
        if sub_name == None:  # eg. "google"
            ret.SetFields(self._GetKeys(org)) # select argument(s)
            ret.Append(self._GetVals(org)) # select argument(s) values
        elif sub_name == u"repos":
            repos = org.get_repos()
            for repo in repos:
                if not ret.GetFields():
                    ret.SetFields(self._GetKeys(repo)) # sets the column names or fields names
